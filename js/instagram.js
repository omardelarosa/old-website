...
$.getJSON("https://api.instagram.com/v1/tags/"+tagInputVal+"/media/recent?access_token="+accessToken+"&scope=likes&callback=?",function(data){   //--- begin JSON data grab---
	...
	boxRandSeqArr.sort(function() {return 0.5 - Math.random()}) //Array elements now scrambled	


	for (var i=1;i<=boxNumTotal;i++){   //creates box LIs with IDs of i
		$('ul #boxes').append('<li></li>');
		};
	...
	$('ul #boxes li:eq('+boxRandSeqArr[i]+')').append('<div class="box" style="" id="'+i+'">&nbsp;</div>'); //adds .box div to random li from boxRandSeqArr

	var boxNum = i+'';

	var unitWidth = 600 / boxWidth;		//calculates (in pixels) box unit Width
	var posX = ((i%boxWidth)*(unitWidth* -1);  //calculates X pos of boxSprite --NEED FIX
/* right most column is totally messed up */
	var posY = (Math.floor(i/boxWidth))*(unitWidth * -1);  //calculates Y pos of box Sprite

	$('#'+boxNum+' div').attr('style','background-repeat:no-repeat;background-image:url('+imgUrl+');background-size:600px 600px;height:'+unitWidth+'px;width:'+unitWidth+'px;background-position:'+posX+'px '+posY+'px;');    // formats wrapper DIV to 600x600px
	...
	$('#userPhoto').html('<a href="'+userProfileUrl+'" target="_blank">'+'<img src="'+userPhotoUrl+'"></img></a>');
